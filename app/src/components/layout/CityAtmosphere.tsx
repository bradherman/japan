...
const CONFIGS: Record<City, {
  ...
  Tokyo:  { color: [96, 165, 250],  count: 30, size: [2, 6],   speed: 0.25, opacity: [0.08, 0.22], type: 'orb' },
  Kyoto:  { color: [192, 132, 252], count: 22, size: [4, 9],   speed: 0.35, opacity: [0.10, 0.25], type: 'petal' },
  Osaka:  { color: [251, 146, 60],  count: 15, size: [10, 28], speed: 0.08, opacity: [0.06, 0.16], type: 'bokeh' },
  Hakone: { color: [74, 222, 128],  count: 12, size: [30, 70], speed: 0.12, opacity: [0.04, 0.10], type: 'mist' },
}
...
function spawn(city: City, w: number, h: number, scatter: boolean): Particle {
  ...
  return {
    x, y, vx, vy, size,
    opacity: scatter ? rand(...c.opacity) * 0.5 : 0,
    targetOpacity: rand(...c.opacity),
...
function draw(ctx: CanvasRenderingContext2D, p: Particle) {
  ...
  switch (c.type) {
    ...
    case 'bokeh': {
      ...
      grad.addColorStop(0.75, `rgba(${r},${g},${b},0.1)`)
      grad.addColorStop(1, `rgba(${r},${g},${b},0)`)
      ctx.fillStyle = grad
      ctx.beginPath()
...
export function CityAtmosphere() {
  ...
  const tick = useCallback((t: number) => {
    ...
    if (target) {
      ...
      if (n < need && Math.random() < Math.max(0.03, (need - n) / need * 0.15)) {
  ...
  useEffect(() => {
    ...
    if (w > 0 && particlesRef.current.filter(p => p.city === city).length === 0) {
      const count = Math.floor(CONFIGS[city].count * 0.6)
  ...
  useEffect(() => {
    ...
    if (c) {
      const { w, h } = sizeRef.current
      const count = Math.floor(CONFIGS[c].count * 0.6)
      for (let i = 0; i < count; i++) {
  ...
  if (reducedMotion) {
    ...
    return (
      <div
        ...
        style={{
          viewTransitionName: 'atmosphere',
          background: c
            ? `radial-gradient(ellipse at 50% 30%, rgba(${c[0]},${c[1]},${c[2]},0.08) 0%, transparent 60%)`
            : undefined,
        }}
        aria-hidden="true"
      />
